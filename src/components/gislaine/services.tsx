...
function parseIncludesList(includes: any): string[] {
  if (!includes) return [];
  if (Array.isArray(includes)) return includes.map((s) => String(s).trim()).filter(Boolean);
...
export function ServicesSection() {
  ...
  // Filter only active services and assign 2-digit sequence number
  ...
  const activeItems = rawItems
    ...
    .map((item: any, index: number) => ({
      ...
      includesList: parseIncludesList(item.includes),
    }));

  const handleCtaClick = (service: any) => {
    ...
    >
      ...
      <div className="relative max-w-5xl mx-auto px-6">
        ...
        >
          ...
          >
            <RenderContent content={services?.titleHtml || 'Serviços e soluções <strong>estratégicas</strong>'} />
          </h2>
        </motion.div>

        <div className="flex flex-col gap-8">
          {activeItems.map((service: any, index: number) => {
            const hasExternalLink = service.ctaLink && !service.ctaLink.startsWith('#');
            const hasAnchor = service.ctaLink && service.ctaLink.startsWith('#');
            ...
            return (
              ...
              >
                ...
                <div className="p-6 sm:p-8 lg:p-10">
                  ...
                  {service.includesList && service.includesList.length > 0 && (
                    <div className="mb-6 p-5 rounded-xl" style={{ backgroundColor: 'rgba(223,130,60,0.06)' }}>
                      ...
                      <ul className="space-y-2.5">
                        {service.includesList.map((item: string, i: number) => (
                  ...
                  <button
                    className="inline-flex items-center gap-2.5 px-6 py-3.5 text-sm font-semibold transition-all duration-300 cursor-pointer"
